    ...
    return (
        <form className={`${styles.contact_form} ${formsuccess ? `${styles.checknow}` : ""}`} id="formid">
            ...
            <div className={`${styles.form_privacy_check}`}>
                ...
                <div className={`txt ${styles.txt}`}><Link href="https://www.hirabari.com/privacy-policy/" target="_blank" rel="noreferrer noopener">プライバシーポリシー<span className={`${styles.icon}`}><FontAwesomeIcon icon={faUpRightFromSquare} /></span></Link>をご一読の上、ご同意頂ける場合は、「同意する」にチェックを入れ送信内容確認画面へお進みください。なお、ご入力いただいた個人情報は当スクールからの連絡やお問い合わせの回答に利用いたします。</div>
                <div className={`${styles.input_box}`}>
            ...
            <div className={`txt ${styles.sub_txt}`}>
                お問い合わせが送信されますと、【受信確認メール】を自動送信いたします。お客さまのご利用環境、また迷惑メール対策等の設定により、お返事が届かない場合があります。「@synapps.com」からのメール受信が可能な設定にしていただきますようお願いいたします。
            </div>
        </form>
    )
}
