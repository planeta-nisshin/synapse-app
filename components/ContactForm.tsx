    ...
    return (
        <form className={`${styles.contact_form} ${formsuccess ? `${styles.checknow}` : ""}`} id="formid">
            ...
            <div className={`${styles.form_privacy_check}`}>
                ...
                <div className={`txt ${styles.txt}`}><a href="https://www.hirabari.com/privacy-policy/" target="_blank" relrel="noopener noreferrer">プライバシーポリシー<span className={`${styles.icon}`}><FontAwesomeIcon icon={faUpRightFromSquare} /></span></a>をご一読の上、ご同意頂ける場合は、「同意する」にチェックを入れ送信内容確認画面へお進みください。なお、ご入力いただいた個人情報は当スクールからの連絡やお問い合わせの回答に利用いたします。</div>
                <div className={`${styles.input_box}`}>
